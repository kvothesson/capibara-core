...
class TestCapibaraClientIntegration:
    ...
    def mock_client_components(self, temp_cache_dir):
        ...
        with patch("capibara.sdk.client.CapibaraClient._initialize_components"):
            ...
            client.cache_manager.get_cache_stats = Mock(
                return_value={
                    "hits": 0,
                    "misses": 0,
                    "evictions": 0,
    ...
    @pytest.mark.asyncio
    async def test_run_script_generation(
        self, mock_client_components, sample_run_response
    ):
        """Test script generation through client."""
        # Arrange
        mock_client_components.engine.run_script = AsyncMock(
            return_value=sample_run_response
        )

        # Act
        response = await mock_client_components.run(
            prompt="print('Hello, World!')", language="python", execute=False
        )
    ...
    async def test_run_script_with_execution(
        ...
        # Act
        response = await mock_client_components.run(
            prompt="print('Hello, World!')", language="python", execute=True
        )
    ...
    @pytest.mark.asyncio
    async def test_run_script_with_context(
        self, mock_client_components, sample_run_response
    ):
        ...
        # Act
        response = await mock_client_components.run(
            prompt="Process these numbers", language="python", context=context
        )
    ...
    @pytest.mark.asyncio
    async def test_run_script_with_security_policy(
        self, mock_client_components, sample_run_response
    ):
        ...
        # Act
        response = await mock_client_components.run(
            prompt="print('Hello, World!')", language="python", security_policy="strict"
        )
    ...
    @pytest.mark.asyncio
    async def test_run_script_with_provider_preference(
        self, mock_client_components, sample_run_response
    ):
        ...
        # Act
        response = await mock_client_components.run(
            prompt="print('Hello, World!')", language="python", llm_provider="groq"
        )
    ...
    async def test_list_scripts(self, mock_client_components):
        ...
        mock_client_components.cache_manager.list_scripts = AsyncMock(
            return_value=mock_scripts
        )

        # Act
        response = await mock_client_components.list_scripts(
            limit=10, offset=0, language="python"
        )
    ...
    async def test_show_script(self, mock_client_components):
        ...
        mock_client_components.cache_manager.get_script = AsyncMock(
            return_value=mock_script
        )

        # Act
        response = await mock_client_components.show_script(
            script_id="script_123", include_code=True
        )
        ...
        # Verify cache manager was called
        mock_client_components.cache_manager.get_script.assert_called_once_with(
            "script_123"
        )
    ...
    async def test_clear_cache(self, mock_client_components):
        ...
        response = await mock_client_components.clear_cache(
            script_ids=["script_1", "script_2"], language="python"
        )
    ...
    async def test_clear_cache_all(self, mock_client_components):
        ...
        # Verify cache manager was called
        mock_client_components.cache_manager.clear_scripts.assert_called_once_with(
            script_ids=None, language=None, older_than=None, all_scripts=True
        )
    ...
    async def test_health_check(self, mock_client_components):
        ...
        # Arrange
        mock_client_components.container_runner.health_check = AsyncMock(
            return_value=True
        )
        mock_client_components.fallback_manager.get_provider_stats = Mock(
            return_value={
                "total_requests": 10,
                "total_successes": 9,
                "total_failures": 1,
    ...
    async def test_health_check_failure(self, mock_client_components):
        ...
        # Arrange
        mock_client_components.cache_manager.get_cache_stats.side_effect = Exception(
            "Cache error"
        )
        mock_client_components.container_runner.health_check = AsyncMock(
            return_value=True
    ...
    def test_get_stats(self, mock_client_components):
        ...
        # Arrange
        mock_client_components.fallback_manager.get_provider_stats = Mock(
            return_value={"total_requests": 10, "success_rate": 90.0, "providers": {}}
        )
        mock_client_components.script_generator.get_generation_stats = Mock(
            return_value={"total_generations": 5, "success_rate_percent": 80.0}
        )

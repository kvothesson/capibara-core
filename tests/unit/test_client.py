...
class TestCapibaraClient:
    ...
    def test_client_initialization(
        ...
        client = CapibaraClient(
            openai_api_key="test_openai_key", groq_api_key="test_groq_key"
        )
    ...
    @patch("capibara.sdk.client.ContainerRunner")
    @patch("capibara.sdk.client.FallbackManager")
    def test_client_initialization_no_api_keys(
        self,
        mock_fallback,
        mock_container,
        mock_policy,
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match="At least one LLM provider API key must be provided"
        ):
            CapibaraClient()
    ...
    async def test_run_method(
        ...
        # Arrange
        with (
            patch("capibara.sdk.client.CapibaraEngine", return_value=mock_engine),
            patch("capibara.sdk.client.CacheManager", return_value=mock_cache_manager),
            patch(
                "capibara.sdk.client.ScriptGenerator",
            ...
            patch(
                "capibara.sdk.client.ContainerRunner",
                return_value=mock_container_runner,
            ),
            patch(
                "capibara.sdk.client.FallbackManager",
            ...
            response = await client.run(
                prompt="test prompt", language="python", execute=False
            )
    ...
    @pytest.mark.asyncio
    async def test_show_script_not_found(
        self,
        mock_engine,
        mock_cache_manager,
        mock_fallback_manager,
    ...
    async def test_health_check(
        ...
        with (
            ...
            mock_fallback_manager.get_provider_stats.return_value = {
                "total_requests": 100
            }
    ...
    def test_get_stats(
        ...
        with (
            ...
            mock_script_generator.get_generation_stats.return_value = {

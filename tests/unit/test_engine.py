...
class TestCapibaraEngine:
    ...
    async def test_run_script_cache_hit(self, engine, sample_request, mock_components):
        ...
        mock_components['cache_manager'].get_script = AsyncMock(return_value=cached_script)
        ...
        mock_components['cache_manager'].increment_cache_hit.assert_called_once_with("test_script_123")
    
    @pytest.mark.asyncio
    async def test_run_script_cache_miss_generation(self, engine, sample_request, mock_components, sample_security_policy):
        ...
        mock_components['script_generator'].generate = AsyncMock(return_value="print('Hello, World!')")
        mock_components['script_generator'].last_provider_used = "openai"
        mock_components['policy_manager'].get_policy.return_value = sample_security_policy
    ...
    @pytest.mark.asyncio
    async def test_run_script_security_scan_failure(self, engine, sample_request, mock_components, sample_security_policy):
        """Test script generation with security scan failure."""
        # Arrange
        mock_components['cache_manager'].get_script = AsyncMock(return_value=None)
        engine.prompt_processor.process = AsyncMock(return_value="processed_prompt")
        mock_components['script_generator'].generate = AsyncMock(return_value="import os; os.system('rm -rf /')")
        mock_components['policy_manager'].get_policy.return_value = sample_security_policy
    ...
    @pytest.mark.asyncio
    async def test_run_script_with_execution(self, engine, sample_request, mock_components, sample_security_policy):
        ...
        mock_components['container_runner'].execute = AsyncMock(return_value=execution_result)
    ...
    def test_modify_python_code_for_execution(self, engine, sample_request):
        ...
        # Act
        modified_code = engine._modify_python_code(code, sample_request.context["inputs"])

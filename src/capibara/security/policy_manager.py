...
class PolicyManager:
    ...
    def get_policy(self, policy_name: Optional[str] = None) -> SecurityPolicy:
    ...
    def _load_builtin_policies(self) -> None:
        ...
        strict_policy = SecurityPolicy(
            ...
            blocked_imports=[
                ...
                "requests", "pickle", "ctypes", "multiprocessing", "threading",
                "eval", "exec", "compile", "__import__"
            ],
            blocked_functions=[
                "eval", "exec", "compile", "__import__", "open", "file", 
                "input", "exit", "quit", "reload"
            ],
        )
        ...
        moderate_policy = SecurityPolicy(
            ...
            blocked_imports=[
                "subprocess", "socket", "urllib", "requests", "pickle", 
                "ctypes", "multiprocessing", "threading", "eval", "exec", 
                "compile", "__import__"
            ],
            blocked_functions=[
                "eval", "exec", "compile", "__import__", "exit", "quit", "reload"
            ],
        )
    ...
    def _load_custom_policies(self) -> None:
        ...
        if not self.policies_dir.exists():
            logger.debug("Policies directory does not exist", dir=str(self.policies_dir))
            return
        
        policy_files = list(self.policies_dir.glob("*.yaml")) + list(self.policies_dir.glob("*.yml"))
        ...
        for policy_file in policy_files:
            try:
                ...
                self.policies[policy.name] = policy
                logger.info("Custom policy loaded", policy=policy.name, file=str(policy_file))
                
            except Exception as e:
                logger.error("Failed to load custom policy", 
                           file=str(policy_file), error=str(e))
    ...
    def save_policy(self, policy: SecurityPolicy, filename: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.error("Failed to save policy", 
                       policy=policy.name, file=str(policy_file), error=str(e))
            raise

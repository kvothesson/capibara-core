        ...
        **kwargs: Any
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning("Groq request timeout", attempt=attempt + 1, max_retries=max_retries)
                if attempt == max_retries:
                    raise LLMProviderError("Groq request timed out")
                
            except Exception as e:
                if "rate_limit" in str(e).lower():
                    logger.warning("Groq rate limit", attempt=attempt + 1, error=str(e))
                    if attempt == max_retries:
                        raise LLMProviderError(f"Groq rate limit exceeded: {str(e)}")

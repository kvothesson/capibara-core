...
class OpenAIProvider(LLMProvider):
    ...
    async def generate_code(self, prompt: str, language: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            logger.error(
                "OpenAI code generation failed", error=str(e), model=self.model
            )
    ...
    async def generate_text(self, prompt: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            logger.error(
                "OpenAI text generation failed", error=str(e), model=self.model
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    raise LLMProviderError("OpenAI request timed out") from e
                ...
                logger.error(
                    "Unexpected OpenAI error", attempt=attempt + 1, error=str(e)
                )
                if attempt == max_retries:

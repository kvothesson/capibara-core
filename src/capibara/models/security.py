...
class AuditEvent(BaseModel):
    ...
    event_type: str = Field(
        ...,
        description="Type of event (script_generated, script_executed, security_violation, etc.)",
    )
    script_id: str | None = Field(None, description="Associated script ID")
...
class ResourceUsage(BaseModel):
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional resource metrics"
    )
...
class SecurityScanResult(BaseModel):
    ...
    passed: bool = Field(
        True, description="Whether scan passed (no blocking violations)"
    )
    scan_duration_ms: int = Field(0, description="Scan duration in milliseconds")
    rules_applied: list[str] = Field(
        default_factory=list, description="Security rules that were applied"
    )
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional scan information"
    )
...
class SandboxConfig(BaseModel):
    ...
    seccomp_profile: str | None = Field(None, description="Seccomp profile")
    apparmor_profile: str | None = Field(None, description="AppArmor profile")
    capabilities: list[str] = Field(
        default_factory=list, description="Linux capabilities"
    )
    metadata: dict[str, Any] = Field(

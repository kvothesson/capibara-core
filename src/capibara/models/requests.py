...
class RunRequest(BaseModel):
    ...
    prompt: str = Field(
        ..., description="Natural language prompt describing the desired script"
    )
    context: dict[str, Any] | None = Field(
        None, description="Additional context for script generation"
    )
    language: str = Field("python", description="Target programming language")
    security_policy: str | None = Field(None, description="Security policy to apply")
    llm_provider: str | None = Field(None, description="Specific LLM provider to use")
    cache_ttl: int | None = Field(3600, description="Cache TTL in seconds")
    resource_limits: dict[str, Any] | None = Field(
        None, description="Custom resource limits"
    )
...
class ListRequest(BaseModel):
    ...
    limit: int = Field(
        50, ge=1, le=1000, description="Maximum number of scripts to return"
    )
    ...
    search: str | None = Field(None, description="Search in prompts and generated code")
...
class ShowRequest(BaseModel):
    ...
    include_code: bool = Field(
        True, description="Include the generated code in response"
    )
    include_execution_logs: bool = Field(
        False, description="Include execution logs if available"
    )
...
class ClearRequest(BaseModel):
    ...
    script_ids: list[str] | None = Field(
        None, description="Specific script IDs to clear"
    )
    language: str | None = Field(
        None, description="Clear all scripts of specific language"
    )
    older_than: int | None = Field(
        None, description="Clear scripts older than N seconds"
    )
    all: bool = Field(False, description="Clear all cached scripts")

    @field_validator("script_ids")
    @classmethod
    def validate_script_ids(cls, v: list[str] | None) -> list[str] | None:

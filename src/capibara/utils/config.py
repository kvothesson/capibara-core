...
class ConfigManager:
    ...
    def _load_from_env(self) -> None:
        ...
        self.config.environment = self._get_env("CAPIBARA_ENVIRONMENT", self.config.environment)
        
        # Database settings
        self.config.database.url = self._get_env("DATABASE_URL", self.config.database.url)
        self.config.database.pool_size = self._get_int_env("DATABASE_POOL_SIZE", self.config.database.pool_size)
        self.config.database.max_overflow = self._get_int_env("DATABASE_MAX_OVERFLOW", self.config.database.max_overflow)
        self.config.database.echo = self._get_bool_env("DATABASE_ECHO", self.config.database.echo)
        ...
        self.config.cache.max_size_mb = self._get_int_env("CACHE_MAX_SIZE_MB", self.config.cache.max_size_mb)
        self.config.cache.cleanup_interval = self._get_int_env("CACHE_CLEANUP_INTERVAL", self.config.cache.cleanup_interval)
        
        # Security settings
        self.config.security.default_policy = self._get_env("DEFAULT_SECURITY_POLICY", self.config.security.default_policy)
        self.config.security.policies_dir = self._get_env("SECURITY_POLICIES_DIR", self.config.security.policies_dir)
        self.config.security.audit_log_dir = self._get_env("AUDIT_LOG_DIR", self.config.security.audit_log_dir)
        self.config.security.enable_audit_logging = self._get_bool_env("ENABLE_AUDIT_LOGGING", self.config.security.enable_audit_logging)
        
        # Container settings
        self.config.container.image = self._get_env("DOCKER_IMAGE", self.config.container.image)
        self.config.container.memory_limit_mb = self._get_int_env("DOCKER_MEMORY_LIMIT_MB", self.config.container.memory_limit_mb)
        self.config.container.cpu_limit = self._get_float_env("DOCKER_CPU_LIMIT", self.config.container.cpu_limit)
        self.config.container.execution_timeout = self._get_int_env("EXECUTION_TIMEOUT", self.config.container.execution_timeout)
        self.config.container.network_access = self._get_bool_env("NETWORK_ACCESS", self.config.container.network_access)
        self.config.container.allow_subprocess = self._get_bool_env("ALLOW_SUBPROCESS", self.config.container.allow_subprocess)
        
        # Logging settings
        self.config.logging.level = self._get_env("LOG_LEVEL", self.config.logging.level)
        self.config.logging.format = self._get_env("LOG_FORMAT", self.config.logging.format)
        self.config.logging.file = self._get_env("LOG_FILE", self.config.logging.file)
        self.config.logging.max_size_mb = self._get_int_env("LOG_MAX_SIZE_MB", self.config.logging.max_size_mb)
        self.config.logging.backup_count = self._get_int_env("LOG_BACKUP_COUNT", self.config.logging.backup_count)
        
        # LLM settings
        self.config.llm.openai_api_key = self._get_env("OPENAI_API_KEY", self.config.llm.openai_api_key)
        self.config.llm.groq_api_key = self._get_env("GROQ_API_KEY", self.config.llm.groq_api_key)
        self.config.llm.default_provider = self._get_env("DEFAULT_LLM_PROVIDER", self.config.llm.default_provider)
        self.config.llm.max_tokens = self._get_int_env("LLM_MAX_TOKENS", self.config.llm.max_tokens)
        self.config.llm.temperature = self._get_float_env("LLM_TEMPERATURE", self.config.llm.temperature)
        self.config.llm.timeout = self._get_int_env("LLM_TIMEOUT", self.config.llm.timeout)
        self.config.llm.retry_attempts = self._get_int_env("LLM_RETRY_ATTEMPTS", self.config.llm.retry_attempts)
        
        # Metrics settings
        self.config.metrics.enabled = self._get_bool_env("METRICS_ENABLED", self.config.metrics.enabled)
        self.config.metrics.port = self._get_int_env("METRICS_PORT", self.config.metrics.port)
        self.config.metrics.path = self._get_env("METRICS_PATH", self.config.metrics.path)
        self.config.metrics.prometheus_endpoint = self._get_env("PROMETHEUS_ENDPOINT", self.config.metrics.prometheus_endpoint)
    ...
    def _validate_config(self) -> None:
        ...
        if self.config.logging.level.upper() not in valid_log_levels:
            logger.warning(f"Invalid log level: {self.config.logging.level}, using INFO")

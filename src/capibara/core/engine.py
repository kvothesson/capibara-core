...
from capibara.models.responses import ExecutionResult, RunResponse
from capibara.models.security import AuditEvent, SecurityScanResult
...
class CapibaraEngine:
    ...
    async def run_script(self, request: RunRequest) -> RunResponse:
        ...
        if cached_script:
            logger.info(
                "Script served from cache", script_id=cached_script["script_id"]
            )
            return await self._build_response_from_cache(cached_script, request)

        # Process prompt
        processed_prompt = await self.prompt_processor.process(
        ...
        if not scan_result.passed:
            logger.warning(
                "Security scan failed", violations=len(scan_result.violations)
            )
            # Log security violations
            await self._log_security_violations(scan_result, request)
            raise SecurityError(
                "Script failed security scan", violations=scan_result.violations
            )
    ...
    async def _build_response_from_cache(
        self, cached_script: dict[str, Any], request: RunRequest
    ) -> RunResponse:
    ...
    async def _execute_script(self, code: str, request: RunRequest) -> ExecutionResult:
        ...
        await self._log_audit_event(
            "script_executed", None, request, execution_result=result
        )

        return result

    async def _log_security_violations(
        self, scan_result: SecurityScanResult, request: RunRequest
    ) -> None:
    ...
    async def _log_audit_event(
        self, event_type: str, script_id: str | None, request: RunRequest, **kwargs: Any
    ) -> None:
        """Log audit event."""
        event = AuditEvent(
    ...
    def _generate_script_id(self) -> str:
        ...
        return f"script_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}_{hashlib.md5().hexdigest()[:8]}"

    def _generate_event_id(self) -> str:
        """Generate unique event ID."""
        return f"event_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}_{hashlib.md5().hexdigest()[:8]}"
        ...
        for func_name in matches:
            if len(python_inputs) >= 3:
                ...
                modified_code = re.sub(
                    pattern_to_replace, replacement, modified_code, count=1
                )

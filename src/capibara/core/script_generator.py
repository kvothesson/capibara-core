...
from capibara.llm_providers.base import LLMProvider
    ...
    async def generate(
        ...
        **kwargs: Any
        ...
        try:
            ...
            generation_prompt = self._build_generation_prompt(prompt, language, **kwargs)
            ...
            logger.error("Script generation failed", error=str(e), provider=provider_name)
            raise ScriptGenerationError(f"Failed to generate script: {str(e)}") from e
    
    def _build_generation_prompt(self, prompt: str, language: str, **kwargs: Any) -> str:

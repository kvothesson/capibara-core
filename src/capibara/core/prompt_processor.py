...
class PromptProcessor:
    ...
    async def process(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if prompt_type in self.prompt_templates:
            processed_prompt = self.prompt_templates[prompt_type].format(prompt=processed_prompt)
        
        # Add safety instructions
        processed_prompt = self._add_safety_instructions(processed_prompt)
        
        logger.debug("Prompt processed", 
    ...
    def _detect_prompt_type(self, prompt: str) -> str:
        ...
        data_keywords = ['process', 'analyze', 'parse', 'transform', 'convert', 'csv', 'json', 'data']
        if any(keyword in prompt_lower for keyword in data_keywords):
            return "data_processing"
        
        # File operations keywords
        file_keywords = ['file', 'read', 'write', 'create', 'delete', 'move', 'copy', 'directory', 'folder']
        if any(keyword in prompt_lower for keyword in file_keywords):
            return "file_operations"
        
        # API integration keywords
        api_keywords = ['api', 'http', 'request', 'endpoint', 'rest', 'graphql', 'fetch', 'post', 'get']
        if any(keyword in prompt_lower for keyword in api_keywords):
            return "api_integration"
        
        # Web scraping keywords
        scraping_keywords = ['scrape', 'crawl', 'extract', 'html', 'website', 'url', 'parse']
        if any(keyword in prompt_lower for keyword in scraping_keywords):
            return "web_scraping"
        
        # Automation keywords
        automation_keywords = ['automate', 'schedule', 'batch', 'loop', 'repeat', 'workflow']
    ...
    def extract_constraints(self, prompt: str) -> List[str]:
        ...
        for pattern in constraint_patterns:
            matches = re.findall(pattern, prompt, re.IGNORECASE)
            for match in matches:
                constraints.extend([constraint.strip() for constraint in match.split(',')])
